...
| `test_field_impact.py` | `assets/field_impact.json`, the per-version field-impact registry the content-pack bundle's phase 900 reads. Shape (every bucket present, a completeness claim, no duplicate versions, no field in two buckets, `meaning_changed` carrying was/now/migration and `mentioned_only` carrying a note). Derivation from `CHANGELOG.md` in BOTH directions: every version section has an entry, every entry names a released version, the SKILL.md frontmatter version is covered, and every `xdm.` path a version's entry names is classified by it. Pins the answers consumers query -- `xdm.auth.service` at 1.9.0 and 1.6.1, `xdm.event.type` at 1.7.0 but NOT at 2.0.2 / 2.0.3, `xdm.target.resource.name` as mandatory rather than meaning, the three banned cloud paths. Plus the `scripts/field_impact.py` exit-code contract, the range union and the exclusive `--from`. | A release landed without declaring its field impact, which reads downstream as 'no impact' and silently validates a migration measurement it should have stopped; or the registry drifted from the changelog it is derived from. |
